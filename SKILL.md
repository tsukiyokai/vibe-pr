...
脚本目录：`~/.claude/skills/canndev/scripts/`
...
每个活跃 PR 有一个 context 文件（`~/.claude/skills/canndev/context/`）记录工作进度和决策。

session 开始时，先读取 context：
```bash
python3 ~/.claude/skills/canndev/scripts/task_context.py --repo <repo> --pr <number>
```

没有 context 文件时，用 `--init` 从当前 PR 状态生成：
```bash
python3 ~/.claude/skills/canndev/scripts/task_context.py --repo <repo> --pr <number> --init
```

不确定在处理哪个 PR 时，列出所有活跃 context：
```bash
python3 ~/.claude/skills/canndev/scripts/task_context.py --list
```
...
如果需求来自 issue：
```bash
python3 ~/.claude/skills/canndev/scripts/issue_parser.py --repo cann/hcomm --issue 123
# 或直接传 URL
python3 ~/.claude/skills/canndev/scripts/issue_parser.py --url https://gitcode.com/cann/hcomm/issues/123
```
...
```bash
python3 ~/.claude/skills/canndev/scripts/pr_status.py --repo cann/hcomm --pr <number>
```
...
```bash
python3 ~/.claude/skills/canndev/scripts/comment_parser.py --repo cann/hcomm --pr <number>
```
...
```bash
python3 ~/.claude/skills/canndev/scripts/reviewer_activity.py --repo cann/hcomm --pr <number> --recent 30
```
...
python3 ~/.claude/skills/canndev/scripts/comment_parser.py --repo cann/hcomm --pr <number> --since-commit
```
...
python3 ~/.claude/skills/canndev/scripts/pr_monitor.py \
...
- state 文件持久化在 `~/.claude/skills/canndev/state/`
...
| 场景 | 命令 |
|------|------|
| 验证 token | `python3 ~/.claude/skills/canndev/scripts/gitcode_api.py` |
| 获取 issue 详情 | `python3 ~/.claude/skills/canndev/scripts/issue_parser.py --repo <repo> --issue <n>` |
| 获取 issue（URL） | `python3 ~/.claude/skills/canndev/scripts/issue_parser.py --url <issue_url>` |
| 查 PR 状态 | `python3 ~/.claude/skills/canndev/scripts/pr_status.py --repo <repo> --pr <n>` |
| 更新 PR 标题/描述 | `gitcode_api.update_pull(repo, token, number, title='...', body='...')` |
| 解析 PR 评论 | `python3 ~/.claude/skills/canndev/scripts/comment_parser.py --repo <repo> --pr <n>` |
| 解析最新 push 后评论 | `python3 ~/.claude/skills/canndev/scripts/comment_parser.py --repo <repo> --pr <n> --since-commit` |
| 分析 reviewer | `python3 ~/.claude/skills/canndev/scripts/reviewer_activity.py --repo <repo> --pr <n> --recent 30` |
| 手动指定候选人 | `python3 ~/.claude/skills/canndev/scripts/reviewer_activity.py --repo <repo> --candidates "a,b,c" --recent 30` |
| 启动 PR 监控 | `python3 ~/.claude/skills/canndev/scripts/pr_monitor.py --repo <repo> --pr <n>` |
| 单次监控（调试） | `python3 ~/.claude/skills/canndev/scripts/pr_monitor.py --repo <repo> --pr <n> --once` |
| 读取 PR 上下文 | `python3 ~/.claude/skills/canndev/scripts/task_context.py --repo <repo> --pr <n>` |
| 初始化 PR 上下文 | `python3 ~/.claude/skills/canndev/scripts/task_context.py --repo <repo> --pr <n> --init` |
| 列出活跃 PR | `python3 ~/.claude/skills/canndev/scripts/task_context.py --list` |
